  ...
  return (
    ...
    >
      ...
      <div className="py-2.5 px-2 space-y-0.5">
        {!collapsed && (
          <div className="px-3 py-1.5 text-[10px] font-semibold tracking-wider text-[#657180] uppercase">
            核心运维导航
          </div>
        )}
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded text-xs transition-colors relative group cursor-pointer ${
                isActive
                  ? 'bg-[#1890FF] text-white font-semibold'
            ...
            >
              ...
              {collapsed && (
                <div className="absolute left-full ml-2 px-2.5 py-1.5 bg-[#1F1F1F] text-white text-xs rounded shadow-md whitespace-nowrap hidden group-hover:block z-50 pointer-events-none">
                  <div className="font-semibold">{item.label}</div>
                  <div className="text-[10px] text-[#8C8C8C]">{item.sublabel}</div>
                </div>
              )}
            </button>
          );
